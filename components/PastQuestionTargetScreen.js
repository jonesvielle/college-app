...
const PastQuestionTargetScreen = ({route, navigation}) => {
  const {title, university, authenticationToken} = route.params;
  ...
  const list2 = ['100', '200', '300', '400', '500', 'POST UTME'];
  const facultyList = [];
  ...
  const handleContinueButton = () => {
    if (selectedFaculty.length < 1 || selectedLevel.length < 1) {
      alert('fill all fields');
    } else {
      // alert('continue');
      navigation.navigate('PastQuestionTargetScreen1', {
        selectedFaculty,
        selectedLevel,
        title,
        university,
  ...
  return (
    <ScrollView
      ...
      <View
        ...
        <Text style={{fontSize: dimension.fontScale * 20, width: '90%'}}>
          Who is this past question for
        </Text>
        <Image
...
export default PastQuestionTargetScreen;
