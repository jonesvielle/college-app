...
const TutorPastQuestionTitle = (route) => {
  const navigation = useNavigation();
  const {university, authenticationToken} = route.route.params;
  ...
  const handleContinueButton = () => {
    if (title.length < 1) {
      ...
      navigation.navigate('PastQuestionTargetScreen', {
        title,
        university,
        authenticationToken,
      });
...
export default TutorPastQuestionTitle;
