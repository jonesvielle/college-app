...
const UplaodSolutionVideoScreen = ({route, navigation}) => {
  const {
    title,
    university,
    selectedLevel,
  ...
  const [solutionVideoOutlines, setVideoSolutionOultlines] = useState([]);
  const [solutionVideoTitle, setSolutionVideoTitle] = useState('');
  ...
  const handleOnChangeVideoTitle = (e) => {
    setSolutionVideoTitle(e);
  };
  ...
  const handleCreateSolutionVideoOutline = () => {
    ...
    if (file.length < 1 || solutionVideoTitle.length < 1) {
      alert('fill all necessary details');
    } else {
      solutionVideoOutlines.push(solutionVideoObject);
      // setClassObjectArray([]);
      // setCourseTitle('');
      setFileObject({name: '.mp4'});
      setFile('');
      setUpdateStates({});
      console.log('output=>', solutionVideoOutlines);

      // setFileObject({});
    }
  };
  const removeSolutionVideoFromArray = (value) => {
    solutionVideoOutlines.splice(value, 1);
    setUpdateStates({});
  };
  ...
  const handleContinueButton = () => {
    if (solutionVideoOutlines.length < 1) {
      alert('fill all fields');
    } else {
      // alert('continue');
      console.log(JSON.stringify(solutionVideoOutlines));
      navigation.navigate('UploadSolutionDocumentScreen', {
        authenticationToken,
        selectedFaculty,
        department,
        selectedLevel,
    ...
    <ScrollView
      ...
      {solutionVideoOutlines.map((c, i) => (
        <View style={{marginTop: '10%'}} key={i}>
          ...
          <View>
            <TextInputComponent
              editable={false}
              placeHolder={c.solutionVideoTitle}
              onChange={handleOnChangeVideoTitle}
            />
          </View>
          <View
            style={{
      ...
      <View style={{marginTop: '10%'}}>
        <View
          ...
          <Text>Solution {solutionVideoOutlines.length + 1}</Text>
        </View>
        <View>
          <TextInputComponent
        ...
        {showFile ? (
          ...
          <View
            ...
            {isUploading ? (
              ...
              <>
                <Text style={{color: 'grey', width: '60%'}}>.mp4</Text>
                <TouchableOpacity
                  onPress={handleUploadFile}
                  style={{
                    backgroundColor: brandColor,
...
export default UplaodSolutionVideoScreen;
