...
const CourseTargetScreen = ({route, navigation}) => {
  const {title, description, courseObjective, university, authenticationToken} =
    route.params;
  ...
  const handleContinueButton = () => {
    if (selectedFaculty.length < 1 || selectedLevel.length < 1) {
      ...
      // alert('continue');
      navigation.navigate('CourseTargetScreen1', {
        selectedFaculty,
        selectedLevel,
        title,
        description,
...
export default CourseTargetScreen;
